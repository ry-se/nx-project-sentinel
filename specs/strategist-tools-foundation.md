# Spec — Strategist Tools: Foundation (plan-drawing, Flow A)

Part of the strategist-tools domain — see `strategist-tools.md` for the full file map.
Covers Waves 0–1 (Flow A: build the operations overlay): the orbit camera, tool
drafting, the `PlanFeature` serializable model, control measures, unit symbols, MGRS
readout, bearings, and plan persistence. Sources: `engine/strategist.ts`,
`engine/planFeature.ts`, `engine/unitSymbol.ts`, `engine/mgrs.ts`, `engine/planStore.ts`.

See also: `strategist-tools-invariants.md` for the cross-cutting invariants every tool
here (and every later-wave tool) MUST hold.
...
metadata — via `Object.prototype.hasOwnProperty.call`, NOT the `in` operator, so a
corrupted `affiliation` equal to a JS built-in key (`"toString"`, `"constructor"`, ...)
cannot resolve to an inherited prototype member instead of a real color (security-review
finding, Wave 4). Selection/rename/delete/undo work automatically via the existing
todo-12 feature-list API — a unit symbol is just another `Feature`.
